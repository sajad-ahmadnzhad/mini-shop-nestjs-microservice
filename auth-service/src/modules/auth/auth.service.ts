...
export class AuthService {
  ...
  sendWelcomeEmail(userEmail: string, username: string): void {
    const args = {
      ...
      html: `<p>hello ${username} welcome to my microservice app.
thank you for your registration.
      </p>`,
    };

    this.notificationServiceClientProxy.emit("send-notification", { ...args });
  }
  ...
  async generateTokens(user: User) {
    ...
    const accessToken = this.jwtService.sign(
      { id: user.id },
      { secret: ACCESS_TOKEN_SECRET, expiresIn: ACCESS_TOKEN_EXPIRE_TIME }
    );
    const refreshToken = this.jwtService.sign(
      { id: user.id },
      { secret: REFRESH_TOKEN_SECRET, expiresIn: REFRESH_TOKEN_EXPIRE_TIME }
    );
    ...
    try {
      ...
      if (existingUser)
        throw new ConflictException(
          "already signup with this username or email"
        );
      ...
      this.sendWelcomeEmail(user.email, user.full_name);
    ...
    try {
      if (!user)
        throw new UnauthorizedException(
          "The information entered is insufficient"
        );
      ...
      this.sendWelcomeEmail(newUser.email, newUser.full_name);
  ...
  async signout(refreshToken: string) {
    try {
      ...
      let decodeToken = this.jwtService.verify<{ id: number }>(refreshToken, {
      ...
      await this.redisCache.del(
        `refreshToken_${decodeToken.id}_${refreshToken}`
      );
